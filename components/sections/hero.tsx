...
export function Hero({ eyebrow, heading, subheading, primaryCta, secondaryCta, children }: HeroProps) {
  return (
      ...
      <div className="relative z-10 grid gap-8 md:grid-cols-2 md:items-center">
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-wide text-slate-600">
            <span className="rounded-full bg-white px-3 py-1 shadow-subtle">Same-day slots</span>
            <span className="rounded-full bg-white px-3 py-1 shadow-subtle">In-house teams</span>
            <span className="rounded-full bg-white px-3 py-1 shadow-subtle">Fully insured</span>
          </div>
        </div>
        {children && <div className="card-surface border-grid relative overflow-hidden p-6 md:p-8">{children}</div>}
      </div>
    </section>
  );
}
